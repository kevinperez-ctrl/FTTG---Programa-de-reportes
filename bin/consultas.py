...
class Consultar(QDialog):
	def __init__(self, user):
		...
		self.gridLayout.addWidget(self.busquedaLabel, 0, 0, 1, 1)
		self.tipobusCombo = QtWidgets.QComboBox(self)
		self.tipobusCombo.addItem("")
		self.tipobusCombo.addItem("")
		self.tipobusCombo.addItem("")
		self.gridLayout.addWidget(self.tipobusCombo, 0, 1, 1, 2)
		self.buscarButton = QPushButton(self)
		self.gridLayout.addWidget(self.buscarButton, 0, 3, 1, 1)
		self.coopLabel = QtWidgets.QLabel(self)
		self.gridLayout.addWidget(self.coopLabel, 2, 0, 1, 1)
		...
		self.gridLayout.addWidget(self.coopCombo, 2, 1, 1, 2)
		self.discEntry = QtWidgets.QSpinBox(self)
		self.discEntry.setMaximum(300)
		self.gridLayout.addWidget(self.discEntry, 2, 3, 1, 1)
		self.discEntry.setEnabled(True)
		self.columButton = QtWidgets.QPushButton(self)
		self.gridLayout.addWidget(self.columButton, 4, 0, 1, 1)
		self.registrosLabel = QtWidgets.QLabel(self)
		self.gridLayout.addWidget(self.registrosLabel, 4, 2, 1, 1)
		self.resultadoLabel = QtWidgets.QLabel(self) 
		self.gridLayout.addWidget(self.resultadoLabel, 4, 3, 1, 1)
		...
		self.gridLayout.addWidget(self.tabla, 5, 0, 1, 7)
	...
	def guardar_datos(self):
		...
		if self.tipobusCombo.currentText() == "Cooperativa":
			...
			if disc == "0" and self.boleteria.currentText() == "":
				tuplaDatos = (coope,)
				conector.execute("SELECT * FROM sanciones WHERE cooperativa=? ORDER BY numero DESC", tuplaDatos)
				...
				if disc == "BOLETERÍA":
					tuplaDatos = (coope,)
					conector.execute("SELECT * FROM sanciones WHERE cooperativa=? and disco='BOLETERÍA' ORDER BY numero DESC", tuplaDatos)
					...
					conector.execute("SELECT * FROM sanciones WHERE cooperativa=? and disco=? ORDER BY numero DESC", tuplaDatos)
			...
			if self.tipobusCombo.currentText() == "Fecha de emisión":
				...
				tuplaDatos = (coope,)
				conector.execute("SELECT * FROM sanciones WHERE fecha=? ORDER BY numero DESC", tuplaDatos)
				...
				if self.tipobusCombo.currentText() == "Número de Boleta":
					...
					tuplaDatos = (coope,)
					conector.execute("SELECT * FROM sanciones WHERE numeroSancion=? ORDER BY numero DESC", tuplaDatos)
